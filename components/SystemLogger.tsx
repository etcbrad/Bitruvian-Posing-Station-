...
export const SystemLogger: React.FC<SystemLoggerProps> = ({ logs, isVisible, onPlayTimelapse, onExportJSON, onClearHistory, historyCount = 0 }) => {
    ...
    >
      ...
      <div ref={logContainerRef} className="overflow-y-auto custom-scrollbar pr-2 h-32">
        {logs.length === 0 ? (
          ...
          logs.map((log, index) => (
            <div key={index} className="flex leading-tight py-0.5 border-b border-ridge/20 last:border-0 group">
              <span className="text-mono-light mr-2 shrink-0 group-hover:text-ink transition-colors">{log.timestamp}</span>
